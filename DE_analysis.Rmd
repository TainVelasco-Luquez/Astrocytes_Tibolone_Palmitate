...
Let's check if technical replicates are indeed replicates, by having simmilar counts per gene:
```{r}

for (i in seq(1, ncol(vali$counts), 2)) {
  
  filename <- str_replace(colnames(vali$counts)[i], pattern = "^[^_]*_", replacement = "")
  filename <- paste0("cor_", filename, ".png")
  
  main_tilte <- paste0(colnames(vali$counts)[i], "_VS_", colnames(vali$counts)[i+1])
  
  png(filename)
    plot(log2(vali$counts[,i]), log2(vali$counts[,i+1]), 
         main = main_tilte)
    abline(a=0, b=1) # Draw the diagonal line
  dev.off()
...
```{r}
LFC_pa_joint_old <- read.csv("Results/Individual_contrasts/alpha0.05_lfcThreshold0.5/LFC_pa_joint_no_outliers.csv")

# With Bonferroni
LFC_pa_joint_new <- read.csv("Results/Individual_contrasts/LFC_pa_joint.csv")

# Genes that were DE and not anymore
length(setdiff(LFC_pa_joint_old$ensembl_gene_id, LFC_pa_joint_new$ensembl_gene_id))
thor <- setdiff(LFC_pa_joint_old$ensembl_gene_id, LFC_pa_joint_new$ensembl_gene_id)

# Genes DE according to old method and lost in the new one
odin <- assay(dds)
dim(odin)
dim(vali$counts)
lost_genes <- vali$counts[rownames(vali$counts) %in% thor,]
dim(lost_genes)
View(lost_genes[, order(sapply(str_split(colnames(vali$counts), "_"), "[", 3))])

# Genes DE according to new method
new_top_genes <- LFC_pa_joint_new %>% arrange(desc(log2FoldChange_pa)) %>% select(ensembl_gene_id) %>% head() %>% unlist()
new_top_genes <- vali$counts[rownames(vali$counts) %in% new_top_genes,]
View(new_top_genes[, order(sapply(str_split(colnames(vali$counts), "_"), "[", 3))])


colnames(vali$counts)[order(sapply(str_split(colnames(vali$counts), "_"), "[", 3))]
```
...
```{r}
my_alpha <- 0.05
palmitato <- results(dds, contrast = c("condition","palmitato", "vehiculo"), alpha = my_alpha, pAdjustMethod = "bonferroni", tidy = T)
names(palmitato)[1] <- "ensembl_gene_id"
palmitato <- filter(palmitato, padj <= my_alpha) %>% arrange(log2FoldChange)
...
write.csv(palmitato, file = "Results/Individual_contrasts/palmitato_vs_vehiculo_alpha0.05.csv", row.names = F)

# Vulcano plot
EnhancedVolcano::EnhancedVolcano(
              palmitato,
              lab = palmitato$external_gene_name,
              x = "log2FoldChange",
              y = "padj",
              FCcutoff = 0,
              pCutoff = my_alpha,
...
palmitato_up_gost <- gprofiler2::gost(palmitato_up$ensembl_gene_id, organism = "hsapiens", ordered_query = T)
palmitato_up_gost_manhattan <- gostplot(palmitato_up_gost, interactive = F, capped = F)
publish_gostplot(p = palmitato_up_gost_manhattan, filename = "Results/Individual_contrasts/palmitato_up_gost_manhattan.jpeg", highlight_terms= palmitato_up_gost$result %>% arrange(p_value) %>% head(15) %>% dplyr::select(term_id))
...
write_csv(palmitato_up_gost, path = "Results/Individual_contrasts/palmitato_up_gost.csv")

palmitato_up_gost_plot <- palmitato_up_gost %>% group_by(source) %>% filter(!source %in% c("TF", "MIRNA", "HPA", "CORUM", "HP")) %>% top_n(10, desc(p_value)) %>% ungroup() %>% arrange(source, p_value)
...
ggsave("Results/Individual_contrasts/palmitato_up_gost_plot.jpg", dpi = "retina", width = 400, height = 200, units = "mm")
...
dim(palmitato_down_gost$result)
```
...
```{r}
my_alpha <- 0.05
joint <- results(dds, contrast = c("condition","TibPal", "vehiculo"), alpha = my_alpha, pAdjustMethod = "bonferroni", tidy = T)
names(joint)[1] <- "ensembl_gene_id"
joint <- filter(joint, padj <= my_alpha) %>% arrange(log2FoldChange)
...
write.csv(joint, file = "Results/Individual_contrasts/joint_vs_vehiculo_alpha0.05.csv", row.names = F)
...
publish_gostplot(p = joint_gost_manhattan, filename = "Results/Individual_contrasts/joint_gost_manhattan.jpeg", highlight_terms= joint_gost$result %>% arrange(p_value) %>% head(10) %>% dplyr::select(term_id))
...
write_csv(joint_gost, path = "Results/Individual_contrasts/joint_gost.csv")

joint_gost_plot <- joint_gost %>% group_by(source) %>% filter(!source %in% c("REAC", "MIRNA", "HPA", "CORUM", "HP", "WP")) %>% top_n(10, desc(p_value)) %>% ungroup() %>% arrange(source, p_value)

ggplot(joint_gost_plot, aes(x = fct_reorder(term_name, p_value), y = -log10(p_value), fill=source), split='source') + geom_col() + theme(axis.text.x=element_text(angle=-40, hjust=0)) + facet_grid(.~source, scale="free_x")
ggsave("Results/Individual_contrasts/joint_gost_plot.jpg", dpi = "retina", width = 400, height = 200, units = "mm")
...
publish_gostplot(p = joint_up_gost_manhattan, filename = "Results/Individual_contrasts/joint_up_gost_manhattan.jpeg", highlight_terms= joint_up_gost$result %>% arrange(p_value) %>% head(10) %>% dplyr::select(term_id))
...
write_csv(joint_up_gost, path = "Results/Individual_contrasts/joint_up_gost.csv")
...
publish_gostplot(p = joint_down_gost_manhattan, filename = "Results/Individual_contrasts/joint_down_gost_manhattan.jpeg", highlight_terms= joint_down_gost$result %>% arrange(p_value) %>% head(10) %>% dplyr::select(term_id))
...
write_csv(joint_down_gost, path = "Results/Individual_contrasts/joint_down_gost.csv")
```

# What is the effect of tibolone

By the difference between joint and PA
```{r}
dim(palmitato)
dim(joint)
length(intersect(palmitato$ensembl_gene_id, joint$ensembl_gene_id))
# The genes present in joint not present in palmitato
joint_diff_pa <- joint[joint$ensembl_gene_id %in% setdiff(joint$ensembl_gene_id, palmitato$ensembl_gene_id),]
dim(joint_diff_pa)

# General enrichment
joint_diff_pa_gost <- gprofiler2::gost(joint_diff_pa$ensembl_gene_id, organism = "hsapiens", ordered_query = T)
joint_diff_pa_gost_manhattan <- gostplot(joint_diff_pa_gost, interactive = F, capped = F)
```

```{r}
my_alpha <- 0.05
palmitato <- results(dds, contrast = c("condition","palmitato", "vehiculo"), pAdjustMethod = "bonferroni", alpha = my_alpha, tidy = T)
names(palmitato)[1] <- "ensembl_gene_id"

joint <- results(dds, contrast = c("condition","TibPal", "vehiculo"), alpha = my_alpha, pAdjustMethod = "bonferroni", tidy = T)
names(joint)[1] <- "ensembl_gene_id"

# Joining only the LFC values for the 30758 genes and excluding about 8945 whose padj is NA because of independent filtering (i.e. after normalisation by the scaling factor, the gene had a low count)
LFC_pa_joint <- left_join(palmitato, joint, by = "ensembl_gene_id", suffix = c("_pa", "_joint")) %>% select(ensembl_gene_id, log2FoldChange_pa, padj_pa, log2FoldChange_joint, padj_joint ) %>% drop_na
...
# Export the table
write.csv(LFC_pa_joint, "Results/Individual_contrasts/LFC_pa_joint.csv", row.names = F)

plot(y = LFC_pa_joint$log2FoldChange_joint, x = LFC_pa_joint$log2FoldChange_pa, xlab="LFC_palmitato", ylab="LFC_joint");abline(h=0, v=0)
...
ggsave("Results/Individual_contrasts/Dumbell_plot_sorted_significantPAonly.png")
...
# second approach
  ...
  hist(LFC_pa_joint$log2FoldChange_diff)
  abline(v = mean(LFC_pa_joint$log2FoldChange_diff), col = "royalblue")
  abline(v = median(LFC_pa_joint$log2FoldChange_diff), col = "royalblue")
  abline(v = quantile(LFC_pa_joint$log2FoldChange_diff, 0.75), col = "royalblue")
...
LFC_pa_joint %>% filter(log2FoldChange_diff >= quantile(log2FoldChange_diff, 0.75)) %>% arrange(desc(log2FoldChange_diff)) %>% write.csv(x=., file = "Results/Individual_contrasts/LFC_pa_joint_no_outliers_significant_in_PA_MostReversed_0.75LFCdiff.csv", row.names = F)
...
write.csv(upGenes_ByPa, file = "Results/Individual_contrasts/upGenes_ByPa.csv", row.names = F)
...
write.csv(downGenes_ByPa, file = "Results/Individual_contrasts/downGenes_ByPa.csv", row.names = F)

### Let's do some enrichments of these genes
## up in PA and reversed by Joint
upGenes_ByPa_gost <- gprofiler2::gost(upGenes_ByPa$ensembl_gene_id, organism = "hsapiens", ordered_query = T, exclude_iea = T)
upGenes_ByPa_gost_manhattan <- gostplot(upGenes_ByPa_gost, interactive = F, capped = F)
publish_gostplot(p = upGenes_ByPa_gost_manhattan, filename = "Results/Individual_contrasts/upGenes_ByPa_gost_manhattan.jpeg", highlight_terms= upGenes_ByPa_gost$result %>% arrange(p_value) %>% head(20) %>% dplyr::select(term_id))
...
write_csv(upGenes_ByPa_gost, path = "Results/Individual_contrasts/upGenes_ByPa_gost.csv")

## down in PA and reversed by Joint
downGenes_ByPa_gost <- gprofiler2::gost(downGenes_ByPa$ensembl_gene_id, organism = "hsapiens", ordered_query = T, exclude_iea = T)
downGenes_ByPa_gost_manhattan <- gostplot(downGenes_ByPa_gost, interactive = F, capped = F)
publish_gostplot(p = downGenes_ByPa_gost_manhattan, filename = "Results/Individual_contrasts/downGenes_ByPa_gost_manhattan.jpeg", highlight_terms= downGenes_ByPa_gost$result %>% arrange(p_value) %>% head(20) %>% dplyr::select(term_id))

downGenes_ByPa_gost <- downGenes_ByPa_gost$result
downGenes_ByPa_gost[sapply(downGenes_ByPa_gost, is.list)] <- apply(
  downGenes_ByPa_gost[sapply(downGenes_ByPa_gost, is.list)],
  1, function(x)
    paste(unlist(x),
      sep = ", ", collapse = ", "
    )
)
write_csv(downGenes_ByPa_gost, path = "Results/Individual_contrasts/upGenes_ByPa_gost.csv")
```
...
<!--
This is an interesting feature for the future: how to assess if the library was deduplicated or not during the initial QC (inmediatly after the sequencing facility) once the gene counts matrix has beed imported into R. In other words, how to assess deduplication looking at the gene counts? 
# Duplication
```{r}
singletons <- sample(letters, 10)
doubletons <- rep(sample(letters, 5), time = 1, each = 2)
sample <- c(singletons, doubletons)
length(unique(sample))
length(unique(singletons))
length(unique(doubletons))

#https://www.biostars.org/p/83842/
